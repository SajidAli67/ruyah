	...
	public function show_item_sales_report()
{
    ...
    if ($q1->num_rows() > 0) {
        ...
        foreach ($q1->result() as $res1) {
            ...
            if ($sales_qty > 0) {
                echo "+ " . format_qty($sales_qty);
            } elseif ($sales_qty < 0) {
                echo "- " . format_qty(abs($sales_qty)); // Display positive quantity with a '-' sign
            } else {
                echo format_qty($sales_qty); // Display zero quantity as-is
            }
            echo "</td>";
        ...
        if ($tot_sales_qty > 0) {
            echo "+ " . format_qty($tot_sales_qty);
        } elseif ($tot_sales_qty < 0) {
            echo "- " . format_qty(abs($tot_sales_qty)); // Display positive quantity with a '-' sign
        } else {
            echo format_qty($tot_sales_qty); // Display zero quantity as-is
        }
        echo "</td>
	...
	public function sales_and_payments_report(){
		...
		$to_date = (!empty($to_date)) ? system_fromatted_date($to_date) : '';
        	...
        	$tot_bal=0;
        			...
        			if(!empty($from_date)){
        				echo "<tr>";
 $result_opening_balance = $this->sales_and_payments_report123($from_date); // Call another function
        		...
        		foreach ($q1->result() as $res2){
        			...
        			// 				: get_sales_payment_details($res2->id);
        							...
        							elseif ("payments") {
        								...
        								get_salesreturn_details($res2->id);
        							}
        			...
        			echo "<tr>";  
	                  ...
	                  if($res2->tr_type!='invoice'){
	                  	...
	                  	$str2 = "<br><b>Return ID :<b>".get_salesreturn_details($details->return_id)->reference_no;
...
// Foter              	// echo "<td>";
		...
		public function sales_and_payments_report123($from_date){
