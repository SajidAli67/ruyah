...
class Pos_model extends CI_Model
{
	...
	{ //Save or update sales
		...
		if(isset($payment_type)){
			$sele_payment_type = $payment_type;
		}
		...
		if ($command == 'update') {
			$sales_entry = array(
				...
				'payment_type'				=> $sele_payment_type
			);
		...
		for ($i = 1; $i <= $payment_row_count; $i++) {
			...
			if ((isset($_REQUEST['amount_' . $i]) && trim($_REQUEST['amount_' . $i]) != '') || ($by_cash == true)) {
				...
				if ($command == 'save' && $pay_all == 'true') {
					...
					// echo "<pre>"; print_r($this->session->userdata('default_account_id'));exit();
		...
		update_account_balance($account_id, $tot_received_amt, true);
		//COMMIT RECORD
