...
class Money_transfer_model_suppler extends CI_Model
{

	//Datatable start
	var $table = 'ac_moneytransfersuppler as a';
	...
	public function verify_and_save()
	{
		...
		if(!empty($debit_account_id)){
			$check_balance  = get_account_balance($debit_account_id);
			$check_balance_byType = get_account_balance_by_type($account_id, $payment_type);
			
			if ($amount > $check_balance) {
				return 'You ' . $payment_type . ' Account balance is less then ' .$amount;
			}

			if ($amount > $check_balance_byType) {
				return 'You Account balance is less then ' . $amount;
			}
		}
