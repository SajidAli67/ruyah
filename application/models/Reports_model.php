...
class Reports_model extends CI_Model
{
	...
	public function show_supplier_items_report()
	{
		...
		if ($q1->num_rows() > 0) {
			...
			$tot_due_amount = 0;
			$opening_stock = 0;
			$item_total_stock = $q1->result()['0']->stock;
			...
			foreach ($q1->result() as $res1) {
				...
				if ($q2->num_rows() > 0) {
					...
					foreach ($q2->result() as $res2) {
						...
						if (warehouse_module() && warehouse_count() > 0) {
							...
							echo "<td hidden>" . get_warehouse_name($res2->warehouse_id) . "</td>";
						}
				...
				// echo "<tr>";
				// $opening_stock  =  $item_total_stock + $tot_sales_qty; 
				// echo "</tr>";
	...
	public function show_purchase_report()
	{
		...
		if (!empty($from_date) && !empty($to_date)) {
			$str1 .= " AND (a.purchase_date >= '" . $from_date . "' AND a.purchase_date <= '" . $to_date . "')";
		}
		...
		if (!empty($from_date) && !empty($to_date)) {
			$str1 .= " AND (b.payment_date >= '" . $from_date . "' AND b.payment_date <= '" . $to_date . "')";
		}
		...
		$str1 .= "SELECT
			...
			d.`created_time_auto` AS time_at
         FROM
		...
		$str1 .= " ORDER BY `tr_date`, `created_time`, `time_at`";
		...
		if (!empty($from_date)) {
			echo "<tr>";
			$result_opening_balance = $this->purchase_and_payments_report123_old($from_date); // Call another function
			echo "<td colspan='4' style='text-align:left'>";
			echo "<h4 class='text-danger'><strong>Today's Opening Balance is: </strong></h4>";
			echo "</td>";
			echo "<td colspan='2' style='text-align:right'>";
			// echo $result_opening_balance;
			echo store_number_format($result_opening_balance);
			echo "</td>";
			echo "</tr>";
		}


		// print_r($q1->num_rows());exit();
		if ($q1->num_rows() > 0) {

			foreach ($q1->result() as $res2) {
				// echo "<pre>"; print_r($q1->result());     			


				// $details = ($res2->tr_type ==	'invoice') ? 

				// 				get_purchase_details($res2->id)

				// 				: get_purchase_payment_details($res2->id);
				...
				if ($res2->tr_type == 'invoice') {
					...
					$this->db->where("a.purchase_id=", $res2->id);
				...
				echo show_date($res2->tr_date) .'<br><small>'.show_time($res2->time_at).'</small>';
				...
				// echo ($res2->tr_type=='invoice') ? "Invoice: ".$res2->tr_code : 'Receive'.$str2." ".$str3;
				if ($res2->tr_type == 'invoice') {
				...
				echo "<td colspan='1' hidden>";

				//echo $res2->tr_code;

				//Show purchase code for the payment

				if ($res2->tr_type == 'payments') {

					echo get_purchase_details(get_purchase_payment_details($res2->id)->purchase_id)->purchase_code;
				}
				...
				// Quantity In purchase_and_payments
				echo "<td colspan='1' hidden>";

				if ($res2->tr_type == 'invoice') {

					$res3 = $this->db->query("select coalesce(sum(purchase_qty),0) as tot_purchase_qty from db_purchaseitems where purchase_id=" . $res2->id);

					echo format_qty($res3->row()->tot_purchase_qty);
				}
				...
				echo "<td  class='text-right' colspan='1'>";
				// print_r("Total Balance".$details->grand_total);exit();
				...
				// Stop8
				// print_r("Total Balance line :".$tot_bal);exit()
				echo store_number_format($tot_bal_updated_with_opening_balance);
				...
				if ($res2->tr_type == 'invoice' or $res2->tr_type == 'return') {

					$tot_qty += $res3->row()->tot_purchase_qty;
		...
		echo "<td > </td>";
		echo "<td colspan='5'>";
		echo "<h4 class='text-danger'><strong>Clousing Balance: </strong></h4>";
		echo "</td>";
		echo "<td class='text-right'>";
		$tot_bal_updated_with_opening_balance = $tot_bal + $result_opening_balance;
		echo store_number_format($tot_bal_updated_with_opening_balance);
		// print_r("Total Balance here: ".$tot_bal);exit();
	...
	public function purchase_and_payments_report123_old($from_date)
	{
		$adjusted_from_date = date('Y-m-d', strtotime('-1 day', strtotime($from_date)));
		// print_r($adjusted_from_date);exit();
		extract($_POST);
		...
		$adjusted_from_date = (!empty($adjusted_from_date)) ? system_fromatted_date($adjusted_from_date) : '';
		...
		if (!empty($from_date) && !empty($adjusted_from_date)) {

			$str1 .= "  and

									(purchase_date>='" . $from_date . "' and purchase_date<='" . $adjusted_from_date . "') ";
		}
		...
		if (!empty($from_date) && !empty($adjusted_from_date)) {
			...
			$str1 .= "  and
									...
									(b.payment_date>='" . $from_date . "' and b.payment_date<='" . $adjusted_from_date . "') ";
		}
		$str1 .= " UNION ";
		...
		if (!empty($from_date) && !empty($adjusted_from_date)) {
			...
			$str1 .= "  and
									...
									(c.return_date>='" . $from_date . "' and c.return_date<='" . $adjusted_from_date . "') ";
		}
		...
		if ($q1->num_rows() > 0) {
			...
			foreach ($q1->result() as $res2) {
				...
				if ($res2->tr_type ==	'invoice') {
					...
					$details = get_purchasereturn_details($res2->id);
				}
				...
				if ($res2->tr_type == 'return') {
					$tot_bal = ($tot_bal + $details->grand_total) - $details->paid_amount;
				}
		...
		echo "<td class='text-right' style='display: none;'>";
		...
		echo store_number_format($tot_bal);
		// print_r("Total Balance here: ".$tot_bal);
		...
		// echo store_number_format($tot_bal);
		// print_r("Today's Opening Balance is: ".$tot_bal);
		return $tot_bal;
	...
	public function show_purchase_report123($from_date)
	{
		...
		if ($q1->num_rows() > 0) {
			...
			// print_r("HGHHGGHGH");exit();
			// console.log($tot_due_amount);
			$total_columns_count = 5;
	...
	public function show_item_sales_report()
	{
		...
		$str1 .= " WHERE a.item_id = " . $item_id;

		if (!empty($from_date) && !empty($to_date)) {
			$str1 .= " and (sales_date>='" . $from_date . "' and sales_date<='" . $to_date . "') ";
		}
        ...
        c.`created_time_auto` AS created_time
        FROM `db_salesitemsreturn` c";

		$str1 .= " WHERE c.item_id = " . $item_id;
        ...
        FROM `db_purchaseitems` d";

		$str1 .= " WHERE d.item_id = " . $item_id;

		if (!empty($from_date) && !empty($to_date)) {
			$str1 .= "  and (d.purchase_date>='" . $from_date . "' and d.purchase_date<='" . $to_date . "') ";
		}
        ...
        FROM `db_purchaseitemsreturn` e";

		$str1 .= " WHERE e.item_id = " . $item_id;

		if (!empty($from_date) && !empty($to_date)) {
			$str1 .= "  and (e.return_date>='" . $from_date . "' and e.return_date<='" . $to_date . "') ";
		}

		$str1 .= " ORDER BY `tr_date`,`created_time` ";
		$q1 = $this->db->query($str1);
		$tot_bal = 0;
		$result_opening_balance = 0;


		if (!empty($from_date)) {
			$result_opening_balance = $this->show_item_sales_report123($from_date);
			$tot_bal = $result_opening_balance;
			echo "<td colspan='2' style='text-align:left'>";
			echo "<b>Today's Opening Stock is: </b>";
			echo "</td>";
			echo "<td colspan='2' style='text-align:right'>";
			echo store_number_format($tot_bal);
			echo "</td>";
		...
		if ($q1->num_rows() > 0) {
			...
			foreach ($q1->result() as $res2) {
				...
				if ($res2->tr_type == 'invoice') {
					echo "Sales Invoice: <a href='" . base_url('sales/invoice/' . get_sales_details($res2->sales_id)->sales_code) . "'>" . get_sales_details($res2->sales_id)->sales_code . "</a>";
				} elseif ($res2->tr_type == 'purchase') {
					echo "Purchase Invoice: <a href='" . base_url('purchase/invoice/' . get_purchase_details($res2->sales_id)->purchase_code) . "'>" . get_purchase_details($res2->sales_id)->purchase_code . "</a>";
				} elseif ($res2->tr_type == 'return') {
					echo "Sale Return Invoice: <a href='" . base_url('sales_return/invoice/' . get_salesreturn_details2($res2->sales_id)->return_code) . "'>" . get_salesreturn_details2($res2->sales_id)->return_code . "</a>";
				} elseif ($res2->tr_type == 'purchase_return') {
					echo "Purchase Return Invoice: <a href='" . base_url('purchase_return/print_invoice/' . get_purchasereturn_details2($res2->sales_id)->id) . "'>" . get_purchasereturn_details2($res2->sales_id)->return_code . "</a>";
				}
				...
				echo "<td colspan='1' hidden>";
				if ($res2->tr_type == 'purchase') {
					echo "<b>Purchase Invoice</b><i>" . "" . "</i><br>";
				} elseif ($res2->tr_type == 'return') {
					echo "<b>Return Invoice</b><i>" . "" . "</i><br>";
				} elseif ($res2->tr_type == 'purchase_return') {
					echo "<b>Purchase Return Invoice</b><i>" . "" . "</i><br>";
				} else {
				...
				if ($res2->tr_type == 'invoice') {
					echo "-" . format_qty($res2->stock_quantity);
					$tot_bal = $tot_bal - $res2->stock_quantity;
				} elseif ($res2->tr_type == 'purchase' || $res2->tr_type == 'return') {
					echo "+" . format_qty(abs($res2->stock_quantity));
					$tot_bal = $res2->stock_quantity + $tot_bal;
				} elseif ($res2->tr_type == 'purchase_return') {
					echo "-" . format_qty(abs($res2->stock_quantity));
					$tot_bal =   $tot_bal - $res2->stock_quantity;
				}
	...
	public function show_item_sales_report123($from_date)
	{
		$adjusted_from_date = date('Y-m-d', strtotime('-1 day', strtotime($from_date)));
		// print_r($adjusted_from_date);exit();
		extract($_POST);
		...
		if (!empty($from_date) && !empty($to_date)) {
			...
			$str1 .= "  and
									...
									(c.return_date>='" . $from_date . "' and c.return_date<='" . $to_date . "') ";
		}
		...
		if (!empty($from_date) && !empty($to_date)) {
			...
			$str1 .= "  and
									...
									(d.purchase_date>='" . $from_date . "' and d.purchase_date<='" . $to_date . "') ";
		}
		...
		if ($q1->num_rows() > 0) {
			...
			foreach ($q1->result() as $res2) {
				if ($res2->tr_type ==	'invoice') {
					$details = get_sales_details($res2->sales_id);
				} elseif ($res2->tr_type ==	'purchase') {
					$details = get_purchase_details($res2->sales_id);
				} else {
					$details = get_salesreturn_details2($res2->sales_id);
				}
				...
				echo show_date($res2->tr_date);
				// echo "<pre>";print_r($res2->sales_id);
				...
				if ($res2->tr_type != 'invoice') {
					...
					$str2 = "<br><b>Purchase Invoice :<b>" . get_purchase_details($res2->sales_id)->purchase_code;
				}
				if ($res2->tr_type == 'invoice') {
					echo "Invoice: <a href='" . base_url('sales/invoice/' . get_sales_details($res2->sales_id)->sales_code) . "'>" . get_sales_details($res2->sales_id)->sales_code . "</a>";
				} elseif ($res2->tr_type == 'purchase') {
					echo "Invoice: <a href='" . base_url('purchase/invoice/' . get_purchase_details($res2->sales_id)->purchase_code) . "'>" . $str2 . "</a>";
				} else {
					echo "Return: <a href='" . base_url('sales_return/invoice/' . $details->return_code) . "'>" . $details->return_code . "</a>";
				}
				echo "</td>";
		...
		// 	echo "Today's Opening Stock 23223is: ";
	...
	public function show_purchase_payments_report()
	{
		...
		if (!empty($from_date)) {
			...
			echo "<td colspan='4' style='text-align:left'>";
			echo "<b>Today's Opening Balance is: </b>";
			echo "</td>";
			echo "<td colspan='2' style='text-align:right'>";
			// echo $result_opening_balance;
			echo store_number_format($result_opening_balance);
	...
	public function show_return_items_report()
	{
		...
		if ($q1->num_rows() > 0) {
			$tot_total_cost = 0;
			$total_columns_count = 7; // Initialize total_columns_count
			...
			foreach ($q1->result() as $res1) {
				...
				$this->db->select(" c.total_cost, c.return_qty, d.item_name ");
			...
			// Print Total after processing all records for this return
			echo "<tr>";
			echo "<td class='text-right text-bold' colspan='$total_columns_count'><b>Total :</b></td>";
			echo "<td class='text-right text-bold'>" . store_number_format($tot_total_cost) . "</td>";
			echo "</tr>";
		} else {
			$total_columns_count = 7;
	...
	public function sales_and_payments_report()
	{
		...
		if ($q1->num_rows() > 0) {
			...
			foreach ($q1->result() as $res2) {
				...
				if ($res2->tr_type == 'invoice') {
					...
					$item_name = $this->db->get()->row()->item_name;
					// $sales_quantity = $this->db->get()->row()->sales_qty;	
					// $avc = $this->db->get()->result();
					// print_r($avc);
				...
				// Quantity In sales_and_payments
				echo "<td colspan='1' hidden>";
		...
		echo "</td>";
		echo "<td colspan='4' ><span class='text-danger'><h4> <strong>Clousing Balance  </strong> </h4></span>";
	...
	public function sales_and_payments_report123_old($from_date)
	{
		$adjusted_from_date = date('Y-m-d', strtotime('-1 day', strtotime($from_date)));
		// print_r($adjusted_from_date);exit();
		extract($_POST);
		...
		if (!empty($from_date) && !empty($adjusted_from_date)) {
			...
			$str1 .= "  and
									...
									(sales_date>='" . $from_date . "' and sales_date<='" . $adjusted_from_date . "') ";
		}
