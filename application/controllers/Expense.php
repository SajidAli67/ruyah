...
class Expense extends MY_Controller {
			...
			$row[] = ($expense->created_by);			
				     $str2 = '<div class="btn-group" title="View Account">
										...
										<ul role="menu" class="dropdown-menu dropdown-light pull-right">';
											...
											$str2.='<li>
												<a  title="Take Print" target="_blank" href="'.base_url().'expense/print_expense/'.$expense->id.'">
	...
	public function print_expense($id){
		$data=$this->data;
		$data['expense']= $this->db->select('e.*, c.category_name')
		->where('e.id',$id)->from('db_expense as e')->join('db_expense_category as c','c.id = e.category_id')->get()->row();
		$data['page_title']=$this->lang->line('expenses');
