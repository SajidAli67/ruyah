...
class Money_deposit extends MY_Controller {
	...
	public function ajax_list()
	{
		...
		foreach ($list as $money_deposit) {
		    ...
		    $customer_detail = get_customer_details($money_deposit->credit_account_id);
			$row[] = $customer_detail->customer_name .' / ' . $customer_detail->customer_code;
			...
			$row[] = ($money_deposit->created_by);			
				     $str2 = '<div class="btn-group" title="View Money Deposit">
										...
										<ul role="menu" class="dropdown-menu dropdown-light pull-right">';
											...
											$str2.='<li>
												<a  title="Take Print" target="_blank" href="'.base_url().'money_deposit/print_money_deposit/'.$money_deposit->id.'">
	...
	public function print_money_deposit($id){
		$data=$this->data;
		$data['deposit']= $this->db->where('id',$id)->get('ac_moneydeposits')->row();
		$data['page_title']=$this->lang->line('deposit');
		 $this->load->view('print_money_deposit',$data);
	}
