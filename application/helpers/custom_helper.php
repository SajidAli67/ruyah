<?php
  ...
  function get_init_code($value,$store_id='',$only_code_flag=false){
   ...
   /* if($value=='journal'){
    ...
    if($value=='suppler_transfer'){
      ...
      $maxid=get_count_id('ac_moneytransfersuppler');
  ...
  function get_purchase_details($purchase_id){
    ...
    return $CI->db->select('*')->from('db_purchase')->where('id',$purchase_id)->get()->row();
  ...
  function get_purchase_payment_details($id){

    $CI =& get_instance();

    return $CI->db->select('*')->from('db_purchasepayments')->where('id',$id)->get()->row();

  }
  function get_salesreturn_details($id){

    // print_r("Testing Custom Helpers");exit();
    $CI =& get_instance();

    return $CI->db->select('*')->from('db_salesreturn')->where('id',$id)->get()->row();

  }
  
  function get_moneydeposits_details($id){
  ...
  function get_salesreturn_details2($id){
    ...
    return $CI->db->select('*')->from('db_salesreturn')->where('sales_id',$id)->get()->row();

  }
function get_purchasereturn_details($id){

    // print_r("Testing Custom Helpers");exit();
    $CI =& get_instance();

    return $CI->db->select('*')->from('db_purchase')->where('id',$id)->get()->row();

  }
  
  function get_purchasereturn_details2($id){

    // print_r("Testing Custom Helpers");exit();
    $CI =& get_instance();

    return $CI->db->select('*')->from('db_purchasereturn')->where('purchase_id',$id)->get()->row();

  } 
  
  function get_purchasereturn_details3($id){

    // print_r("Testing Custom Helpers");exit();
    $CI =& get_instance();

    return $CI->db->select('*')->from('db_purchasereturn')->where('id',$id)->get()->row();
